...
- **`PreToolUse` (Grep/Glob/Read)** — las tools nativas de Claude Code no son "crudas" (son el diseño correcto), así que aquí nunca se bloquea: cada 5 usos seguidos recuerda que `youmindag references`/`architect` traen además bóveda + grafo + historial, no solo el match/archivo. `Read` solo cuenta si el archivo es código fuente (`.ts`/`.tsx`/`.js`/…) — leer la bóveda (`.md`) o config no es un salto de protocolo, así que no genera ruido. Cubre el caso de leer un archivo de código directamente (a mano o vía un subagente) sin pasar antes por `references`, aunque ya se conozca la ruta exacta.
