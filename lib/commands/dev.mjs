...
import { existsSync, readFileSync, writeFileSync, mkdirSync, openSync, closeSync, appendFileSync, rmSync } from 'fs'
import { join, dirname } from 'path'
import { execSync, spawn } from 'child_process'
import { RESET, CYAN, GREEN, YELLOW } from '../utils.mjs'
import { readYoumindagData, writeYoumindagData } from '../vault.mjs'
    ...
    if (existingPid) {
      ...
      if (Number.isInteger(pid) && pid > 0) {
        try {
          ...
          console.log(`${YELLOW}🔪 Dev server anterior detenido (PID ${pid})${RESET}`)
        } catch {}
