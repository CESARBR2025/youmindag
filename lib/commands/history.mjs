...
export function cmdHistory(cwd, args) {
  ...
  if (showAll || showSessions) {
    ...
    if (showJson) {
      output.sessions = recentSessions
  ...
  if (showAll || showDecisions) {
    ...
    if (filter) {
      const re = new RegExp(filter, 'i')
      filtered = filtered.filter(e => re.test(e.text || '') || re.test(e.rationale || ''))
    }
    const recentDecisions = filtered.slice(-recent)

    if (showJson) {
      output.decisions = recentDecisions
      ...
      for (const d of recentDecisions) {
        ...
        console.log(`  ${CYAN}[${ts}]${RESET} ${YELLOW}${d.text || ''}${RESET}`)
        if (d.rationale) {
