...
import { getBovedaDir, readYoumindagVersion } from '../vault.mjs'
import { PKG_VERSION } from '../version.mjs'
import { uninstallClaudeLayer } from '../claude.mjs'
import { checkStaleness } from '../staleness.mjs'
import { cleanGitignoreEntries } from '../gitignore.mjs'
import { findProjectFiles } from './sync.mjs'
...
export function cmdUninstall(cwd, version) {
  ...
  if (existsSync(join(cwd, '.git'))) {
    try {
      ...
      const dirty = runGit(cwd, ['status', '--porcelain', `${bovedaGitPath}/`, '.opencode/', 'scripts/', 'AGENTS.md'])
      if (dirty) {
  ...
  readline.question(`  ${YELLOW}¿Continuar con la desinstalación? (y/N)${RESET} `, (answer) => {
    ...
    try {
      for (const r of uninstallClaudeLayer(cwd)) console.log(`  ${GREEN}✅ ${r}${RESET}`)
    } catch {}
...
export function checkStaleBoveda(cwd, returnResult = false) {
  ...
  if (!result) return returnResult ? null : undefined

  if (returnResult) return result

  if (result.mode === 'per-feature') {
    if (result.stale.length === 0) return undefined
    console.log(`  ${YELLOW}⚠️  ${result.stale.length}/${result.evaluated} feature${result.stale.length === 1 ? '' : 's'} de la bóveda podría${result.stale.length === 1 ? '' : 'n'} estar desactualizado${result.stale.length === 1 ? '' : 's'} (el código cambió después que el doc):${RESET}`)
    for (const s of result.stale) {
      console.log(`  ${YELLOW}   • ${s.feature}.md — ${s.count} commit${s.count === 1 ? '' : 's'} atrasado${s.count === 1 ? '' : 's'}${RESET}`)
    }
    console.log()
  } else {
    // legacy: ningún Feature doc tenía tabla de Componentes parseable
    console.log(`  ${YELLOW}⚠️  La bóveda parece ${result.count} commit${result.count === 1 ? '' : 's'} atrasada respecto al código (chequeo global, impreciso — agrega tablas de Componentes a los docs para un chequeo por feature)${RESET}`)
    console.log()
  }
  return undefined
}
...
export function showHelp(version) {
  ...
  console.log(`${BOLD}🔍 Descubrimiento & Diagnóstico:${RESET}`)
  ...
  console.log(`  npx youmindag architect <modulo>          Cargar contexto completo de un módulo (bóveda + grafo + historial)`)
  console.log(`  npx youmindag architect                   Listar módulos disponibles en la bóveda`)
  console.log(`  npx youmindag doctor                      Diagnóstico completo del proyecto`)
  console.log(`  npx youmindag doctor --json               Diagnóstico en formato JSON`)
  console.log(`  npx youmindag guide                       Guía visual de la arquitectura YouMindAG`)
  console.log(`  npx youmindag guide boveda                Explicación de la bóveda`)
  console.log(`  npx youmindag guide graphify              Explicación del grafo de código`)
  console.log(`  npx youmindag history                     Últimos eventos y decisiones registradas`)
  console.log(`  npx youmindag history --decisions         Solo decisiones/ADRs`)
  console.log(`  npx youmindag history --sessions          Solo sesiones`)
  console.log(`  npx youmindag history --recent 20         Últimas 20 entradas`)
  console.log(`  npx youmindag history --filter "patron"   Filtrar por término`)

  console.log(`\n${BOLD}⚙️  Operaciones:${RESET}`)
  ...
  console.log(`  npx youmindag enforce                     Estado del enforcement para Claude Code`)
  console.log(`  npx youmindag enforce --install           Instalar hooks + CLAUDE.md + skill (Claude Code)`)
  console.log(`  npx youmindag enforce --uninstall         Retirar solo las entradas youmindag`)
  ...
  console.log(`\n${CYAN}${BOLD}💡 Flujo recomendado:${RESET} ${GREEN}youmindag architect <modulo>${RESET} → ${GREEN}youmindag doctor${RESET} → ${GREEN}youmindag sync${RESET}`)
  console.log(`${CYAN}💡 Ayuda específica:${RESET} ${GREEN}youmindag help architect${RESET} │ ${GREEN}youmindag help --examples${RESET}\n`)
}

export function cmdHelp(cwd, args) {
  const topic = args.find(a => !a.startsWith('-'))

  if (args.includes('--examples')) return showExamples()

  if (!topic) return showHelp(PKG_VERSION)

  switch (topic.toLowerCase()) {
    case 'architect':
      console.log(`\n${BOLD}youmindag architect — Protocolo Arquitecto${RESET}\n`)
      console.log(`${CYAN}Uso:${RESET}`)
      console.log(`  ${GREEN}youmindag architect${RESET}                    Listar módulos disponibles en la bóveda`)
      console.log(`  ${GREEN}youmindag architect <modulo>${RESET}           Contexto de un módulo (bóveda + grafo + historial)`)
      console.log(`  ${GREEN}youmindag architect <modulo> --full${RESET}    Contexto completo sin truncar`)
      console.log(`  ${GREEN}youmindag architect --guide${RESET}            Guía del protocolo arquitecto\n`)
      console.log(`${CYAN}Flujo:${RESET}`)
      console.log(`  1. Lee la documentación del módulo en la bóveda`)
      console.log(`  2. Consulta el grafo de dependencias (graphify)`)
      console.log(`  3. Muestra decisiones y sesiones previas\n`)
      console.log(`${CYAN}Ejemplo:${RESET} ${GREEN}youmindag architect infracciones${RESET}\n`)
      break
    case 'doctor':
      console.log(`\n${BOLD}youmindag doctor — Diagnóstico del Proyecto${RESET}\n`)
      console.log(`${CYAN}Uso:${RESET}`)
      console.log(`  ${GREEN}youmindag doctor${RESET}               Diagnóstico completo en formato legible`)
      console.log(`  ${GREEN}youmindag doctor --json${RESET}         Diagnóstico en JSON (para scripts/parseo)\n`)
      console.log(`${CYAN}Qué verifica:${RESET}`)
      console.log(`  • Bóveda poblada y actualizada`)
      console.log(`  • Graphify instalado y grafo generado`)
      console.log(`  • AGENTS.md con reglas`)
      console.log(`  • Scripts disponibles`)
      console.log(`  • Historial (sesiones + decisiones)`)
      console.log(`  • Código muerto detectado\n`)
      break
    case 'history':
      console.log(`\n${BOLD}youmindag history — Historial de Sesiones y Decisiones${RESET}\n`)
      console.log(`${CYAN}Uso:${RESET}`)
      console.log(`  ${GREEN}youmindag history${RESET}                    Últimas 5 sesiones + decisiones`)
      console.log(`  ${GREEN}youmindag history --sessions${RESET}          Solo eventos de sesión`)
      console.log(`  ${GREEN}youmindag history --decisions${RESET}         Solo decisiones técnicas (ADRs)`)
      console.log(`  ${GREEN}youmindag history --recent 20${RESET}          Últimas 20 entradas`)
      console.log(`  ${GREEN}youmindag history --filter "bug"${RESET}       Filtrar por término\n`)
      break
    case 'guide':
      console.log(`\n${BOLD}youmindag guide — Arquitectura Visual${RESET}\n`)
      console.log(`${CYAN}Uso:${RESET}`)
      console.log(`  ${GREEN}youmindag guide${RESET}               Diagrama completo bóveda ↔ graphify ↔ CLI`)
      console.log(`  ${GREEN}youmindag guide boveda${RESET}         Solo la bóveda de conocimiento`)
      console.log(`  ${GREEN}youmindag guide graphify${RESET}       Solo el grafo de código\n`)
      break
    case 'sync':
    case 'status':
    case 'watch':
    ...
    case 'uninstall':
    case 'help':
      return showHelp(PKG_VERSION)
    default:
      console.log(`${YELLOW}Comando "${topic}" no reconocido.${RESET}`)
      console.log(`${YELLOW}Usa${RESET} ${GREEN}youmindag help${RESET} ${YELLOW}para ver todos los comandos.${RESET}\n`)
  }
}

function showExamples() {
  console.log(`\n${BOLD}${CYAN}🧠 YouMindAG — Ejemplos de Uso${RESET}\n`)

  console.log(`${BOLD}📋 Flujo Típico de un Arquitecto:${RESET}`)
  console.log(`  ${GREEN}$ youmindag architect infracciones${RESET}`)
  console.log(`     → Lee boveda/🧩 Features/Infracciones.md`)
  console.log(`     → Consulta graphify query "infracciones"`)
  console.log(`     → Muestra últimas decisiones y sesiones`)
  console.log(`  ${GREEN}$ youmindag doctor${RESET}`)
  console.log(`     → Verifica que bóveda, grafo, scripts estén al día`)
  console.log(`  ${GREEN}$ youmindag sync${RESET}`)
  console.log(`     → Actualiza grafo y repuebla bóveda tras cambios\n`)

  console.log(`${BOLD}🔍 Investigación de Código:${RESET}`)
  console.log(`  ${GREEN}$ youmindag references validateInfraccion${RESET}`)
  console.log(`     → Encuentra todas las referencias de esa función en el proyecto`)
  console.log(`  ${GREEN}$ youmindag context --load pagos${RESET}`)
  console.log(`     → Muestra documentación y archivos del módulo "pagos"\n`)

  console.log(`${BOLD}📜 Historial:${RESET}`)
  console.log(`  ${GREEN}$ youmindag history --decisions --recent 10${RESET}`)
  console.log(`     → Últimas 10 decisiones técnicas registradas`)
  console.log(`  ${GREEN}$ youmindag history --filter "pago"${RESET}`)
  console.log(`     → Buscar en sesiones y decisiones por "pago"\n`)
}
