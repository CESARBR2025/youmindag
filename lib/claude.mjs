...
// Si el settings.json del usuario está corrupto, NO lo toca.
export function mergeClaudeSettings(cwd) {
  ...
  for (const [event, entries] of Object.entries(HOOK_ENTRIES)) {
    ...
    if (!existing.some(entryHasYmHook)) {
  ...
  return { ok: true, message: `hooks agregados en ${added} evento${added === 1 ? '' : 's'}` }
}
...
export function claudeLayerStatus(cwd) {
  ...
  const scriptsOk = ['ym-hook-guard.mjs', 'ym-hook-session-start.mjs', 'ym-hook-posttool.mjs']
